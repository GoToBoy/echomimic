...
# 设置基础输出路径
# TODO: change path
BASE_OUTPUT_DIR = Path("/root/autodl-fs/echomimic_output")
# 在代码开始处确保基础目录存在
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="./configs/prompts/animation_acc_upScale.yaml")
...
def main():
    ...
    save_dir_name = f"step_{args.steps}-cfg_{args.cfg}-facemusk_{args.facemusk_dilation_ratio}-facecrop_{args.facecrop_dilation_ratio}-{args.W}"
    save_dir = BASE_OUTPUT_DIR / date_str / save_dir_name
    ...
    for ref_image_path in config["test_cases"].keys():
        for audio_path in config["test_cases"][ref_image_path]:
            ...
            video = video
            temp_video_path = f"{save_dir}/temp_{ref_name}_{audio_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}.mp4"
            save_videos_grid(
                video,
                temp_video_path,
                n_rows=1,
            ...
            final_video_path = f"{save_dir}/{ref_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}.mp4"
            video_clip.write_videofile(final_video_path, codec="libx264", audio_codec="aac")
            os.remove(temp_video_path)
            print(f"{save_dir}/{ref_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}_withaudio.mp4")
